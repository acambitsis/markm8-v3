...
- The user wants **explicit, on-demand invocation** (`/command` syntax)
- It's a **simple, single-purpose prompt** (fits in one file)
...
- Supports arguments (`$ARGUMENTS`, `$1`, `$2`)
- Can include file contents with `@` prefix
- Can run bash commands with `!` prefix
...
- Runs in **separate context window** (doesn't bloat main conversation)
...
- It's a **reusable expertise package** that applies contextually
...
Use !bash-command to run shell commands before execution.
```
