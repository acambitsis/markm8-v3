...
// Fallback threshold (always matches since min: 0)
const FALLBACK_THRESHOLD: GradeThreshold = {
  min: 0,
  hex: '#ef4444',
  bg: 'bg-red-500',
  text: 'text-red-600',
...
export function getGradeThreshold(percentage: number): GradeThreshold {
  return GRADE_THRESHOLDS.find(t => percentage >= t.min) ?? FALLBACK_THRESHOLD;
}
