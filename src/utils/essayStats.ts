...
/**
 * Get a description of vocabulary diversity
 */
export function getVocabularyDescription(diversity: number): string {
  if (diversity >= 0.7) {
    return 'Excellent';
  }
  if (diversity >= 0.6) {
...
/**
 * Get a description of sentence variety
 */
export function getSentenceVarietyDescription(
  ...
  const variance
    = sentenceLengths.reduce((sum, len) => sum + (len - avg) ** 2, 0)
